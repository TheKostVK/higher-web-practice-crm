 ...
 */
export const useTableSort = <TSortField extends string, TRecord = Record<string, unknown>>() => {
    const [sortBy, setSortBy] = useState<TSortField | undefined>();
    const [order, setOrder] = useState<TSortOrder>();

    const handleTableChange: NonNullable<TableProps<TRecord>['onChange']> = (_pagination, _filters, sorter) => {
