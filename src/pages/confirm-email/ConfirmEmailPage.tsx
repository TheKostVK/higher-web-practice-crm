...
export const ConfirmEmailPage = () => {
    ...
    const handleConfirmEmail = async () => {
        // TODO: добавить запрос подтверждения почты
    };

    const handleResend = () => {
        // TODO: добавить повторную отправку письма
    };

    return (
        <section className={Styles.confirmEmailPage}>
            <h2 className={Styles.confirmEmailPage__title}>Подтверждение почты</h2>
            <p className={Styles.confirmEmailPage__description}>
                Вставьте ссылку из полученного письма
            </p>
            <form
                className={Styles.confirmEmailPage__form}
                onSubmit={handleSubmit(handleConfirmEmail)}
                noValidate
            >
                <label className={Styles.confirmEmailPage__field} htmlFor="confirm-email-link">
                    <span className={Styles.confirmEmailPage__label}>Ссылка подтверждения <span aria-hidden="true">*</span></span>
                    <Controller
                        control={control}
                    ...
                    {errors.confirmationLink && (
                        <span className={Styles.confirmEmailPage__error}>
                            {errors.confirmationLink.message}
                        </span>
                    )}
                </label>
